...
def explore():
    ...
    next_url = (
        url_for("main.explore", page=activities.next_num)
        if activities.has_next
        else None
    )
    prev_url = (
        url_for("main.explore", page=activities.prev_num)
...
def user(username):
    ...
    next_url = (
        url_for("main.user", username=user.username, page=activities.next_num)
        if activities.has_next
        else None
    )
    prev_url = (
        url_for("main.user", username=user.username, page=activities.prev_num)
        if activities.has_prev
        else None
    )
    return render_template(
